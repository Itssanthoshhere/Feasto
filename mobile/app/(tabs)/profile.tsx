...
import { Package, MapPin, ChevronRight, LogOut } from 'lucide-react-native';
          ...
          <View className="items-center py-8 bg-gradient-to-b from-orange-50 to-white border-b border-slate-100">
            {user?.image ? (
              ...
              <View className="w-24 h-24 rounded-full bg-gradient-to-br from-[#FF5A1F] to-[#D9480F] items-center justify-center border-4 border-white shadow-md">
                <Text className="text-4xl text-white" style={{ fontFamily: 'Outfit_800ExtraBold' }}>
                  {firstLetter}
                </Text>
              </View>
            )}
            <Text className="text-2xl text-slate-800 mt-4" style={{ fontFamily: 'Outfit_700Bold' }}>
              {user?.name || 'User'}
            </Text>
            <Text className="text-slate-500 mt-1" style={{ fontFamily: 'Outfit_400Regular' }}>
              {user?.email}
            </Text>
            {user?.role && (
              <View className="mt-3 bg-orange-50 border border-orange-200 px-4 py-1.5 rounded-full">
                <Text className="text-[#FF5A1F] text-xs uppercase tracking-widest" style={{ fontFamily: 'Outfit_700Bold' }}>
                  {user.role}
                </Text>
              </View>
            )}
            ...
            {menuItems.map((item) => (
              ...
              >
                <View className="flex-row items-center gap-4">
                  ...
                  <Text className="text-slate-700" style={{ fontFamily: 'Outfit_600SemiBold' }}>
                    {item.label}
                  </Text>
                </View>
        ...
        <View className="bg-white rounded-3xl overflow-hidden shadow-sm border border-slate-100">
          ...
          >
            <View className="flex-row items-center gap-4">
              ...
              <Text className="text-red-600" style={{ fontFamily: 'Outfit_600SemiBold' }}>
                Logout
              </Text>
            </View>
          </TouchableOpacity>
