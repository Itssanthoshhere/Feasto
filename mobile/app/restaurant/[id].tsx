    ...
    Promise.all([
      ...
      .catch((e: any) => {
        if (e?.response?.status === 404) {
          ...
          setError(
            e?.response?.data?.message || "Failed to fetch restaurant data",
          );
        }
      })
      .finally(() => {
    ...
    <SafeAreaView className="flex-1 bg-slate-50">
      <ScrollView
        contentContainerStyle={{
          paddingBottom:
            cart && cart.length > 0 && cart[0].restaurantId._id === id
              ? 100
              : 20,
        }}
      >
        {/* Hero image */}
        <View className="h-52 bg-orange-50 relative">
          {restaurant.image ? (
            <>
              <Image
                ...
                resizeMode="cover"
              />
              {/* Dark scrim only over real photos */}
            ...
            <View className="w-full h-full items-center justify-center bg-orange-50">
              ...
              />
              <Text
                className="text-[#FF5A1F] text-lg mt-2"
                style={{ fontFamily: "Outfit_700Bold" }}
              >
                Feasto
              </Text>
        ...
        <View className="p-4 gap-4">
          ...
          <View className="bg-white rounded-3xl p-5 shadow-sm border border-slate-100">
            <View className="flex-row items-start justify-between mb-3">
              <Text
                className="text-2xl text-slate-900 flex-1 mr-2"
                style={{ fontFamily: "Outfit_800ExtraBold" }}
              >
                {restaurant.name}
              </Text>
              <View
                className={`px-3 py-1 rounded-full ${restaurant.isOpen ? "bg-green-50 border border-green-200" : "bg-red-50 border border-red-200"}`}
              >
                <Text
            ...
            <View className="bg-orange-50 rounded-3xl p-4 border border-orange-200">
              ...
              <View className="gap-2">
                {promotions.map((promo) => (
                  <View
                    key={promo._id}
                    className="bg-white rounded-2xl p-3 flex-row items-center justify-between border border-orange-100"
                  >
                    <View>
                      <Text
                    ...
                    <View className="bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-lg">
                      <Text
                        className="text-slate-700 text-xs tracking-widest"
                        style={{ fontFamily: "Outfit_700Bold" }}
                      >
                        {promo.code}
                      </Text>
      ...
      {/* Floating Cart Bar */}
      {cart && cart.length > 0 && cart[0].restaurantId._id === id && (
        <View className="absolute bottom-8 left-4 right-4 shadow-lg shadow-black/20">
          <TouchableOpacity
            onPress={() => router.push("/(tabs)/cart")}
            className="bg-[#FF5A1F] rounded-2xl flex-row items-center justify-between p-4 px-6 shadow-sm"
          >
            <View>
              <Text
                ...
                style={{ fontFamily: "Outfit_700Bold" }}
              >
                {quantity} item{quantity > 1 ? "s" : ""} | ₹{subTotal}
              </Text>
              <Text
